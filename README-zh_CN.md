...
<a href="https://gibbok.github.io/typescript-book/zh-cn/plus-edition/"><img src="https://raw.githubusercontent.com/gibbok/typescript-book/main/website/public/images/plus-edition-cover_en.webp" alt="《The Concise TypeScript Book》Plus 版封面" width="320" /></a>
...
- [The Concise TypeScript Book](#the-concise-typescript-book)
  ...
  - [其他](#其他)
    ...
    - [satisfies 操作符](#satisfies-操作符)
    - [仅类型导入和导出](#仅类型导入和导出)
    - [使用声明和显式资源管理](#使用声明和显式资源管理)
      - [await using 声明](#await-using-声明)
    - [导入属性](#导入属性)
    - [正则表达式语法检查](#正则表达式语法检查)
...
`tsconfig.json` 文件用于配置 TypeScript 编译器 (`tsc`)。通常，它会与 `package.json` 一起放在项目根目录中。
...
`lib` 属性用于指定编译时要包含哪些库文件。TypeScript 会自动包含 `target` 属性所指定功能的 API，但可以根据特定需求省略或选择特定库。例如，如果您正在开发服务器项目，则可以排除 `DOM` 库，因为它只在浏览器环境中有用。

#### strict

`strict` 选项通过启用更严格的类型检查来提高类型安全性。从 TypeScript 6.0 开始，该选项默认启用；否则，您需要在 `tsconfig.json` 文件中显式将其设置为 `true`。启用 `strict` 后，TypeScript 可以：
...
`module` 属性用于设置编译器支持的模块系统。运行时，模块加载器会根据指定的模块系统查找并执行依赖项。
...
`jsx` 属性仅适用于 React 中使用的 `.tsx` 文件，并控制 JSX 构造如何编译为 JavaScript。一个常见的选项是 `preserve`，它会编译为 `.jsx` 文件，保持 JSX 不变，以便可以将其传递给 Babel 等不同工具进行进一步转换。

#### skipLibCheck

`skipLibCheck` 属性会阻止 TypeScript 对导入的第三方包进行类型检查。此属性可以减少项目的编译时间。TypeScript 仍会根据这些包提供的类型定义检查您的代码。
...
`include` 属性向编译器指示我们想要包含的文件列表。此属性允许类似 glob 的模式，例如 `*` 表示任何子目录，`_` 表示任何文件名，`?` 表示可选字符。
...
TypeScript 在为某些高级或低级 JavaScript 功能生成代码时会使用辅助代码。默认情况下，这些辅助代码会复制到使用它们的文件中。`importHelpers` 选项会从 `tslib` 模块导入这些辅助代码，从而使 JavaScript 输出更加高效。

### 迁移到 TypeScript 的建议

对于大型项目，建议采用渐进式过渡，其中 TypeScript 和 JavaScript 代码最初共存。只有小型项目才适合一次性迁移到 TypeScript。

此转变的第一步是将 TypeScript 引入构建流程。这可以通过使用 `allowJs` 编译器选项来完成，该选项允许 `.ts` 和 `.tsx` 文件与现有 JavaScript 文件共存。由于 TypeScript 无法从 JavaScript 文件推断类型时会回退到 `any`，因此建议在迁移开始时在编译器选项中禁用 `noImplicitAny`。

第二步是确保您的 JavaScript 测试与 TypeScript 文件一起工作，以便您可以在转换每个模块时运行测试。如果您正在使用 Jest，请考虑使用 `ts-jest`，它允许您使用 Jest 测试 TypeScript 项目。

第三步是在项目中包含第三方库的类型声明。这些声明可以在第三方库的类型声明文件或专门的声明包中找到，您可以通过 [https://www.typescriptlang.org/dt/search](https://www.typescriptlang.org/dt/search) 搜索并安装它们：
...
第四步是使用自下而上的方法逐个模块地迁移，遵循从叶开始的依赖关系图。这个想法是开始转换不依赖于其他模块的模块。要可视化依赖关系图，您可以使用 `madge` 工具。

有一些对转换为 TypeScript 比较友好的模块（外部 API 或规范相关的实用函数和代码），比如 Swagger、GraphQL 或 JSON Schema，它们可以自动生成 TypeScript 类型定义，并在项目中使用。
...
在迁移过程中，您可以使用 `@ts-check` 指令，该指令在 JavaScript 文件中启用 TypeScript 类型检查。该指令提供了宽松版本的类型检查，最初可用于识别 JavaScript 文件中的问题。当 `@ts-check` 包含在文件中时，TypeScript 将尝试使用 JSDoc 风格的注释来推断定义。但是，仅在迁移的早期阶段考虑使用 JSDoc 注释。

考虑在您的 `tsconfig.json` 文件中将 `noEmitOnError` 设置为 false，即使报告错误，这也将允许您输出 JavaScript 源代码。
...
TypeScript 的结构类型系统是基于 JavaScript 的动态鸭子类型系统在运行时的工作方式而设计的。
...
在这个上下文中，`extends` 关键字可以被看作是“子集”。它为类型设置了一个约束。当 `extends` 与泛型一起使用时，它会将泛型类型参数约束为一个更具体的类型。

请注意，这里的 `extends` 与 OOP 意义上的类继承无关。

TypeScript 使用结构类型，并没有严格的名义层级。事实上，如下面的示例所示，两个类型可以相互重叠，而其中任何一个都不是另一个的子类型，因为 TypeScript 考虑的是对象的结构，或者说形状。
