...
<a href="https://gibbok.github.io/typescript-book/bg-bg/plus-edition/"><img src="website/public/images/plus-edition-cover_en.webp" alt="Корица на разширеното издание на The Concise TypeScript Book" width="320" /></a>
...
Ако безплатната книга ви е помогнала да отстраните грешка, да разберете сложна концепция или да напреднете в кариерата си, моля, обмислете да подкрепите работата ми, като платите колкото желаете, с препоръчителен принос от **$5**, или като ме почерпите с кафе.

Вашата подкрепа ми помага да поддържам съдържанието актуално и да го разширявам с нови примери, по-ясни обяснения и допълнителни практически насоки.
...
- [The Concise TypeScript Book](#the-concise-typescript-book)
  - [Безплатна и с отворен код](#безплатна-и-с-отворен-код)
  - [Разширеното издание на The Concise TypeScript Book](#разширеното-издание-на-the-concise-typescript-book)
  - [Подкрепете проекта](#подкрепете-проекта)
  - [Преводи](#преводи)
...
Книгата обхваща TypeScript 7.0.
...
TypeScript 7.0 беше пренаписан в Go като нативна имплементация на компилатора и езиковата услуга. Той използва многонишковост със споделена памет и други оптимизации, за да направи пълните изграждания и функционалностите в редактора по-бързи, като намалява времето за обратна връзка по време на разработка.

Някои функции за производителност в TypeScript 7.0 могат да се настройват. Проверката на типовете може да се изпълнява в паралелни worker-и с `--checkers`; повече worker-и могат да ускорят големи проекти, но използват повече памет. Преработеният режим `--watch` подобрява наблюдението на файлове между различни платформи. TypeScript 7.0 все още не включва compiler API (към юли 2026 г.), така че инструменти, които все още се нуждаят от TypeScript 6.0 API, могат да работят паралелно с TypeScript 7.0 чрез `@typescript/typescript6` или npm aliases.
...
Свойството "target" се използва, за да се определи в коя версия на ECMAScript трябва да се генерира/компилира вашият TypeScript код. За съвременните браузъри ES6 е добър избор. Забележка: Поддръжката на ES5 беше обявена за остаряла в TypeScript 6.0 и вече не се поддържа в TypeScript 7.0.
...
Забележка: Поддръжката на по-старите модулни системи (AMD, UMD, SystemJS) беше обявена за остаряла в TypeScript 6.0 и вече не се поддържа в TypeScript 7.0.

#### moduleResolution

Свойството "moduleResolution" определя стратегията за разрешаване на модули. Използвайте "nodenext" или "bundler" за модерен TypeScript код. Стратегията "classic" се използва само за стари версии на TypeScript (преди 1.6).
...
В TypeScript 6.0 някои по-стари конфигурационни опции и синтактични форми бяха обявени за остарели или преминаваха през старо поведение. В TypeScript 7.0 те са директни грешки или поведение без ефект.

Остарелите възможности, които са се превърнали в директни грешки с поведение без ефект, са:
...
* деактивиране на `esModuleInterop` или `allowSyntheticDefaultImports`
* деактивиране на `alwaysStrict`
* ключовата дума `module` в декларации на namespace
* `asserts` при imports
* `/// <reference no-default-lib />` под `skipDefaultLibCheck`
* файлови пътища в CLI с локален `tsconfig.json`, освен ако не се използва `--ignoreConfig`
...
Ключавое слова `extends` у гэтым кантэксце можна разглядаць як "падмноства". Яно задае абмежаванне для тыпу. Калі `extends` выкарыстоўваецца з generic, яно абмяжоўвае generic-параметр тыпу больш канкрэтным тыпам.

Звярніце ўвагу, што `extends` тут не мае нічога агульнага з наследаваннем класаў у сэнсе ААП.

TypeScript працуе са структурнымі тыпамі і не мае строгай намінальнай іерархіі. На самай справе, як у прыкладзе ніжэй, два тыпы могуць перакрывацца, пры гэтым ніводзін з іх не з’яўляецца падтыпам іншага, таму што TypeScript улічвае структуру, або форму, аб’ектаў.
...
Тып можна пашырыць з дапамогай інтэрфейсу, але не наадварот:
